...
def build_daily_report_message(report_date=None):
    report_date = report_date or now_est().date().isoformat()
    with state_lock:
...
def daily_report_scheduler():
    """يرسل تقريراً يومياً عند 4:15 PM بتوقيت نيويورك"""
    while True:
        try:
            now = now_est()
            # التقرير يرسل مرة واحدة فقط في اليوم عند 4:15 PM
            if now.hour == 16 and now.minute >= 15:
                with state_lock:
                    ...
                    if last_sent != today:
                        # بناء التقرير
                        perf = state.get("performance", {"wins": 0, "losses": 0, "total_pnl": 0.0})
