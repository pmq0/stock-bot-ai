...
# ================= WEBULL SETUP =================
def setup_webull():
    """Initializes and logs into the Webull account using environment variables."""
    wb = webull()
    ...
    if not phone or not password:
        ...
        return None
    try:
        logger.info("Attempting to log in to Webull...")
        wb.login(phone=phone, password=password)
        mfa = wb.get_mfa()
        if mfa:
            logger.warning(f"MFA required. Please check your authenticator app for the code: {mfa}")
        accounts = wb.get_accounts()
        if accounts:
            logger.info(f"Webull login successful. First account ID: {accounts[0].get('accountId')}")
            positions = wb.get_positions()
            if positions:
                logger.info(f"Current positions found: {len(positions)}")
            else:
                logger.info("No current positions found.")
        else:
            logger.warning("No accounts found for this Webull user.")
        return wb
    except Exception as e:
...
def place_order_with_webull(wb, symbol, action, quantity, order_type='MKT', price=None):
    ...
    if wb is None:
        logger.error("Webull client is not initialized.")
        return None
    try:
        if order_type == 'MKT':
            logger.info(f"Attempting to {action} {quantity} shares of {symbol} at market.")
            # order = wb.place_order(symbol=symbol, action=action, quant=quantity, orderType=order_type)
            pass
        elif order_type == 'LMT' and price:
            logger.info(f"Attempting to {action} {quantity} shares of {symbol} at limit ${price}.")
            pass
        else:
...
PENNY_MIN_PRICE = 0.3
PENNY_MAX_PRICE = 10.0  # تم التعديل لصيد الأسهم الرخيصة فقط
PENNY_MIN_GAIN = 25.0   # تم الرفع إلى 25% لصيد المتفجرات فقط
PENNY_MIN_RVOL = 4.0    # تم الرفع لضمان حجم تداول انفجاري

# (بقية الكود تم دمجه بشكل نظيف)
