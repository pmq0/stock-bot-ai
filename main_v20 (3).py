...
def update_all_tickers():
    ...
    tickers = []
    try:
        logger.info("🔄 Fetching tickers from fast GitHub mirror...")
        # مصدر سريع وموثوق لقائمة أسهم ناسداك (تجنب مشاكل Timeout في Railway)
        url = "https://raw.githubusercontent.com/r1b/ticker-symbols/main/data/nasdaq_tickers.csv"
        
        response = requests.get(url, timeout=15)
        if response.status_code == 200:
            content = response.text
            lines = content.strip().split('\n')
            for line in lines[1:]:  # تخطي عنوان العمود
                parts = line.split(',')
                if parts:
                    symbol = parts[0].strip().upper()
                    # فلتر: رمز بين 1 و 5 أحرف، أبجدي فقط
        ...
        if len(clean) > 1000:
            ...
            logger.warning(f"Got only {len(clean)} tickers, using fallback.")
