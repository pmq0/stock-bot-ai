...
def fast_filter(symbol):
    ...
    try:
        ...
        # فلتر السعر: تم توسيع النطاق ليشمل الأسهم النشطة حتى 50 دولار
        if price < 0.5 or price > 50:
            return False

        # فلتر حجم التداول: خفض الحد الأدنى لـ 100 ألف لضمان عدم تفويت الأسهم المتوسطة (مثل WNW)
        if volume < 100000:
            return False
...
PHASE_SETTINGS = {
    "PRE":     {"min_score": 75,  "size_multiplier": 0.5, "vol_surge_mult": 2.0, "description": "🟡 Pre-Market"},
    "REGULAR": {"min_score": 75,  "size_multiplier": 1.0, "vol_surge_mult": 1.5, "description": "🟢 Regular Hours"},
    "AFTER":   {"min_score": 85,  "size_multiplier": 0.3, "vol_surge_mult": 2.5, "description": "🔵 After-Hours"},
...
def detect_explosion(df):
    ...
    # انفجار: الحجم الحالي أكثر من 2x المتوسط (تم خفضه من 3x لضمان عدم تفويت الفرص)
    spike = last_vol > avg_vol * 2

    # زخم قوي: السعر الحالي أعلى من أعلى سعر في آخر 5 شمعات
    momentum = df["close"].iloc[-1] >= df["high"].iloc[-6:-1].max()
...
def process_symbol(symbol):
    try:
        ...
        if score >= effective_min_score or (is_silent_acc and score >= effective_min_score - 10):
            now = time.time()
