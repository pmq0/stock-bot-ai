...
STRONG_NEWS_KEYWORDS = [
    "acquisition", "merger", "partnership", "contract awarded",
    "fda approval", "fda approved", "breakthrough", "buyout",
    "record revenue", "beats estimates", "raised guidance",
    "strategic agreement", "joint venture", "exclusive license"
]
...
# ================= FAST EXPLOSION SCANNER SETTINGS =================
EXPLOSION_CHECK_INTERVAL = 15      # يفحص كل 15 ثانية
EXPLOSION_MIN_GAIN = 10.0          # أقل ارتفاع 10% في الدقيقة
EXPLOSION_MAX_PRICE = 30.0         # سعر معقول للمضاربة
EXPLOSION_MIN_VOLUME = 100000      # حجم لا يقل عن 100 ألف
EXPLOSION_COOLDOWN = 300           # 5 دقائق بين الإشارات لنفس السهم
...
# ================= FAST EXPLOSION SCANNER GLOBALS =================
_last_prices = {}                  # متغيرات لتخزين الأسعار السابقة
_last_prices_lock = threading.RLock()
...
def rss_catalyst_scanner():
    ...
    IMPORTANT_KEYWORDS = [
        # عقود واتفاقيات
        "agreement", "contract", "partnership", "collaboration", "joint venture",
        "enters into", "development services", "service agreement",
        # استحواذات
        "acquisition", "merger", "buyout", "takeover", "acquired",
        # موافقات regulators
        "fda approval", "fda clearance", "breakthrough", "authorization", "approved",
        # نتائج مالية
        "record revenue", "record earnings", "beats estimates", "positive results",
        # أخرى
        "exclusive license", "strategic agreement", "major contract", "uplisting",
        "clinical supply", "clinical trial", "positive data", "successful trial",
    ]
            ...
            for feed_url in RSS_FEEDS:
                ...
                for news_entry in feed.entries[:20]:
                    try:
                        ...
                        # استخرج رمز السهم من العنوان - طرق متعددة
                        symbol = None
                        
                        # الطريقة 1: ابحث عن NASDAQ: أو NYSE: متبوعاً برمز
                        nasdaq_match = re.search(r'NASDAQ:([A-Z]{1,5})|NYSE:([A-Z]{1,5})', title)
                        if nasdaq_match:
                            symbol = nasdaq_match.group(1) or nasdaq_match.group(2)
                        
                        # الطريقة 2: ابحث عن رمز بين قوسين (ABC)
                        if not symbol:
                            paren_match = re.search(r'\(([A-Z]{1,5})\)', title)
                            if paren_match:
                                symbol = paren_match.group(1)
                        
                        # الطريقة 3: ابحث عن أي كلمة من 2-5 حروف كبيرة
                        if not symbol:
...
if __name__ == "__main__":
    ...
    threading.Thread(target=fast_explosion_scanner, daemon=True).start()  # ⚡ ماسح الانفجار السريع (10-15% في دقيقة)
                ...
                try:
                    ...
                    for entry in feed.entries[:15]:
                        try:
                            ...
                            # استخراج رمز السهم من العنوان
                            # أولاً: البحث عن نمط (XXXX) بين قوسين
                            symbol_match = re.search(r'\(([A-Z]{2,5})\)', entry.get('title', ''))
                            
                            if symbol_match:
                                symbol = symbol_match.group(1)
                            else:
                                # إن لم يوجد، استخرج أول كلمة بحروف كبيرة (2-5 أحرف)
                            ...
                            if not symbol:
                                logger.debug(f"⚠️ تعذر استخراج رمز السهم من: {entry.get('title', '')[:50]}")
                                continue
...
# ======================== FAST EXPLOSION SCANNER ========================
# يصطاد الأسهم اللي ترتفع 10-15% في دقيقة واحدة

def fast_explosion_scanner():
    """ماسح الانفجار السريع - يكتشف الأسهم اللي تقفز 15% خلال دقيقة"""
    global _last_prices
    ...
    while True:
        try:
            ...
            with state_lock:
                tickers = list(state.get("tickers", []))[:300]  # أسرع 300 سهم
            ...
            for symbol in tickers:
                try:
                    # جلب آخر دقيقة (شمعة 1-minute)
                    df = cached_download(symbol, period="1d", interval="1m", timeout=5, force_refresh=True)
                    ...
                    # فلتر السعر
                    if current_price < 0.5 or current_price > EXPLOSION_MAX_PRICE:
                        continue
                    
                    # فلتر الحجم
                    if current_volume < EXPLOSION_MIN_VOLUME:
                        continue
                    
                    # حساب التغير من دقيقة مضت
                    with _last_prices_lock:
                        last_data = _last_prices.get(symbol, {})
                        last_price = last_data.get('price', current_price)
                        last_time = last_data.get('time', current_time)
                        
                        # احسب الفرق الزمني بالثواني
                        time_diff = (current_time - last_time).total_seconds()
                        
                        # إذا مر أقل من 30 ثانية من آخر فحص، خلها تمر
                        if time_diff < 30 and last_price > 0:
                            gain_pct = (current_price - last_price) / last_price * 100
                            
                            # تحقق من الانفجار
                            if gain_pct >= EXPLOSION_MIN_GAIN:
                                # منع التكرار
                                signal_key = f"explode_{symbol}_{int(current_time.timestamp() / EXPLOSION_COOLDOWN)}"
                                with state_lock:
                                    if signal_key in state.get("seen_signals", {}):
                                        continue
                                
                                # حساب الزخم الإضافي
                                volume_spike = current_volume / max(df['volume'].tail(5).mean(), 1)
                                
                                # الأهداف السريعة
                                entry = current_price
                                tp1 = round(entry * 1.08, 3)   # +8% خلال 5-10 دقائق
                                tp2 = round(entry * 1.15, 3)   # +15%
                                sl = round(entry * 0.97, 3)    # -3%
                                
                                # تحديد قوة الانفجار
                                if gain_pct >= 20:
                                    strength = "💥 *انفجار عنيف جداً!*"
                                elif gain_pct >= 15:
                                    strength = "⚡ *انفجار قوي جداً!*"
                                elif gain_pct >= 12:
                                    strength = "🔥 *انفجار قوي!*"
                                else:
                                    strength = "📈 *حركة سريعة!*"
                                ...
                                msg = (
                                    ...
                                    f"📊 *{symbol}*\n"
                                    f"💰 السعر: *${entry:.3f}*\n"
                                    f"📈 القفزة: *+{gain_pct:.1f}%* في أقل من دقيقة\n"
                                    f"🔥 الحجم: {int(current_volume):,} ({volume_spike:.1f}x)\n"
                                    f"━━━━━━━━━━━━━━━━\n"
                                    f"🎯 الهدف الأول: *${tp1}* (+8%)\n"
                                    f"🎯 الهدف الثاني: *${tp2}* (+15%)\n"
                                    f"🛑 وقف الخسارة: *${sl}* (-3%)\n"
                                    f"━━━━━━━━━━━━━━━━\n"
                                    f"💡 *اشتر فوراً واخرج بسرعة!*\n"
                                    f"⚠️ *هذا سهم شديد الخطورة - مضاربة فقط*"
                                )
                                
                                send_telegram(msg)
                                logger.info(f"⚡ EXPLOSION: {symbol} +{gain_pct:.1f}% in {time_diff:.0f}s vol={volume_spike:.1f}x")
                                
                                with state_lock:
                                    state.setdefault("seen_signals", {})[signal_key] = time.time()
                                    save_state()
                                
                                time.sleep(2)  # راحة بعد الإشارة
                    ...
                    logger.debug(f"Explosion scanner error {symbol}: {sym_err}")
                    continue
                
                time.sleep(0.1)  # سرعة عالية بين الأسهم
                
        except Exception as e:
            logger.error(f"Fast explosion scanner error: {e}")
        
        time.sleep(EXPLOSION_CHECK_INTERVAL)  # كل 15 ثانية
