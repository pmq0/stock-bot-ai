...
STRONG_NEWS_FEEDS = [
    ...
    # "https://www.prnewswire.com/rss/news-releases-list.rss",  # dead 404
...
def final_trade_recommendation(symbol, news_impact=0, setup="NONE", pump_risk=False, send_alert=True):
    ...
    try:
        ...
        if not can_send_ai_request() or not OPENAI_AVAILABLE or client is None:
            ...
            return base
        prompt = f"""Analyze {symbol}: Price {price:.2f}, RVOL {rvol:.1f}, RSI {rsi:.0f}, Above VWAP {above_vwap}, Tech Score {technical_score}/100.
Return JSON ONLY: {{"decision":"BUY|WATCH|AVOID","confidence":0-100,"setup":"...","risk_level":"...","reasons":[],"invalidations":[],"reason_ar":"..."}}"""
        response = groq_chat_completion(
            model=AI_MODEL,
            messages=[
                {"role": "system", "content": "Conservative analyst. JSON ONLY. NO MARKDOWN."},
                {"role": "user", "content": prompt},
            ], response_format={"type": "json_object"}, temperature=0.1, max_tokens=250, timeout=15)
    ...
    while True:
        try:
            ...
            for c in candidates[:30]:
                ...
                if tech_score >= 40:
                    try:
                        time.sleep(3.0) # Avoid Yahoo 429
                        ticker = yf.Ticker(symbol)
                        info = ticker.info
