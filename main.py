...
# ================= WEBULL SETUP =================
def setup_webull():
    """Initializes and logs into the Webull account with anti-bot measures."""
    import uuid
    wb = webull()
    
    # محاكاة جهاز حقيقي لتجنب حظر Railway IP
    device_id = str(uuid.uuid4())
    wb._did = device_id
    wb._headers['User-Agent'] = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
    ...
    if not email or not password:
        ...
        return None
    try:
        logger.info(f"Attempting to log in to Webull with email: {email} (DID: {device_id[:8]})...")
        # محاولة تسجيل الدخول
        res = wb.login(email, password)
        
        # إذا طلب رمز تحقق (MFA)
        if isinstance(res, dict) and res.get('msg') == 'MFA Required':
            msg = "⚠️ *Webull MFA Required*\nيرجى التحقق من بريدك الإلكتروني وإدخال رمز التحقق باستخدام الأمر:\n`/mfa 123456`"
            send_telegram(msg)
            logger.warning("Webull login requires MFA.")
            return wb
            
        accounts = wb.get_accounts()
        if accounts:
            logger.info(f"Webull login successful. First account ID: {accounts[0].get('accountId')}")
            send_telegram("✅ *Webull Connected*\nتم ربط حساب Webull بنجاح والتداول الآلي مفعل!")
        return wb
    except Exception as e:
        err_msg = f"❌ *Webull Connection Failed*\nError: {e}"
        logger.error(f"Webull login failed: {e}")
        send_telegram(err_msg)
        return None

def place_order_with_webull(wb, symbol, action, quantity, order_type='MKT', price=None):
    """ينفذ أمر شراء أو بيع حقيقي في Webull"""
    if wb is None:
        logger.warning("Webull client not initialized. Skipping order.")
        return None
    try:
        logger.info(f"🚀 Executing Webull Order: {action} {quantity} {symbol} ({order_type})")
        # تحويل الأكشن إلى صيغة ويبول (BUY / SELL)
        wb_action = 'BUY' if action.upper() == 'BUY' else 'SELL'
        
        # جلب الـ Account ID
        accounts = wb.get_accounts()
        if not accounts:
            logger.error("No Webull accounts found.")
            return None
        account_id = accounts[0].get('accountId')
        
        # تنفيذ الأمر (سوقي بشكل افتراضي لضمان السرعة مع المتفجرات)
        res = wb.place_order(
            stock=symbol,
            tId=None,
            action=wb_action,
        ...
        logger.info(f"✅ Webull Order Result: {res}")
        return res
    except Exception as e:
        logger.error(f"❌ Failed to place Webull order for {symbol}: {e}")
        return None

def volatility_swing_scanner():
    """يراقب كل الأسهم المتذبذبة ويرسل تنبيهات شراء عند القاع وبيع عند القمة آلياً"""
    try:
        with state_lock:
            # فحص أول 200 سهم من القائمة النشطة
        ...
        for symbol in tickers:
            ...
            # حساب التذبذب (ATR أو نطاق السعر)
            high_day = df['high'].max()
            low_day = df['low'].min()
            range_pct = (high_day - low_day) / low_day * 100
            
            # نركز فقط على الأسهم المتذبذبة (تحركت أكثر من 10% اليوم)
            if range_pct < 10:
                continue
            ...
            # 🟢 إشارة شراء (Dip): RSI تحت 30 (قاع القناة)
            if current_rsi < 30 and last_action != "BUY":
                msg = f"📉 *فرصة شراء (Buy the Dip): {symbol}*\n"
                msg += f"━━━━━━━━━━━━━━━━\n"
                msg += f"💰 السعر: *${price:.2f}*\n"
                msg += f"📊 تذبذب اليوم: {range_pct:.1f}%\n"
                msg += f"📉 RSI: {current_rsi:.1f} (قاع)\n"
                msg += f"💡 السهم في منطقة هبوط مبالغ فيه، توقع ارتداداً!"
                send_telegram(msg)
                with state_lock:
                    state.setdefault("seen_signals", {})[signal_key] = "BUY"
                    save_state()
                    
            # 🔴 إشارة بيع (Rip): RSI فوق 70 (قمة القناة)
            elif current_rsi > 70 and last_action != "SELL":
                msg = f"📈 *فرصة بيع (Sell the Rip): {symbol}*\n"
                msg += f"━━━━━━━━━━━━━━━━\n"
                msg += f"💰 السعر: *${price:.2f}*\n"
                msg += f"📊 تذبذب اليوم: {range_pct:.1f}%\n"
                msg += f"📈 RSI: {current_rsi:.1f} (قمة)\n"
                msg += f"💡 السهم وصل لقمة الموجة، فكر في جني الأرباح!"
                send_telegram(msg)
                with state_lock:
...
def fast_momentum_scanner():
    ...
    # استدعاء ماسح التذبذب كجزء من المسح السريع
    volatility_swing_scanner()
...
def open_trade(symbol, price, score, df, is_accumulating=False, is_pre_breakout=False, phase="REGULAR", settings=None, rvol=1.0, low_float=False, is_bullish=True, is_pdh=True, is_silent_acc=False, is_raw_mom=False, psych_level=0, gap_pct=0, is_v_shape=False, is_whale_acc=False, is_bb_squeeze=False, effective_min_score=None):
    ...
    with state_lock:
        ...
        # 🚀 تنفيذ أمر شراء حقيقي في Webull
        wb_res = place_order_with_webull(webull_client, symbol, 'BUY', size)
...
def close_trade(symbol, price, reason):
    with state_lock:
        ...
        # 🚀 تنفيذ أمر بيع حقيقي في Webull
        place_order_with_webull(webull_client, symbol, 'SELL', trade["size"])
...
if bot:
    ...
    @bot.message_handler(commands=['mfa'])
    def cmd_mfa(message):
        if not ensure_authorized(message):
            return
        try:
            ...
            if len(args) != 2:
                send_telegram("Usage: /mfa 123456")
                return
            code = args[1]
            if webull_client:
                res = webull_client.login_mfa(os.getenv('WEBULL_EMAIL'), os.getenv('WEBULL_PASSWORD'), code)
                send_telegram(f"🔄 Webull MFA Result: {res}")
                # محاولة جلب الحسابات للتأكد من نجاح الدخول
                accounts = webull_client.get_accounts()
                if accounts:
                    send_telegram("✅ *Webull MFA Successful*\nتم ربط الحساب بنجاح!")
            else:
                send_telegram("❌ Webull client not initialized.")
        except Exception as e:
            send_telegram(f"❌ MFA Error: {e}")
...
def fast_filter(symbol):
    """فلتر أولي سريع للأسهم — يحمي من الضغط الزائد على Yahoo"""
    try:
        df = cached_download(symbol, period="1d", interval="5m")
        if df.empty or len(df) < 5:
            return False
        ...
        if price < 0.3 or price > 30:
            return False

        if price < 5 and volume < 50000:
            return False
        elif price < 20 and volume < 100000:
            return False
        elif volume < 200000:
            return False
...
def rss_catalyst_scanner():
    """ماسح مخصص لصيد الأسهم التي ترتفع بناءً على أخبار إيجابية قوية (VERU-style)."""
    RSS_FEEDS = [
        "https://www.benzinga.com/feed",
        "https://www.businesswire.com/rss/home/?rss=G1",
        "https://www.prnewswire.com/rss/news-releases-list.rss",
    ...
    IMPORTANT_KEYWORDS = [
        ...
        "exclusive license", "strategic agreement", "joint venture",
        "major contract", "uplisting", "record revenue", "positive results",
    ]
    ...
    while True:
        try:
            for feed_url in RSS_FEEDS:
                ...
                for news_entry in feed.entries[:20]:
                    try:
                        title = news_entry.get('title', '')
                        title_lower = title.lower()

                        # هل الخبر يحتوي على كلمة مفتاحية قوية؟
                        match = any(kw in title_lower for kw in IMPORTANT_KEYWORDS)
                        if not match:
                            continue

                        # استخرج رمز السهم من العنوان
                        symbol_match = re.search(r'\(([A-Z]{1,5})\)', title)
                        if not symbol_match:
                            words = title.split()
                            symbol_match = re.search(r'\b([A-Z]{2,5})\b', words[0]) if words else None
                        if not symbol_match:
                            continue

                        symbol = symbol_match.group(1).upper()

                        # تجاهل كلمات شائعة غير رموز أسهم
                        exclude = {'THE', 'AND', 'FOR', 'INC', 'LLC', 'LTD', 'CEO', 'CFO',
                                   'IPO', 'FDA', 'SEC', 'NYSE', 'USD', 'ETF', 'RSS', 'NEW'}
        ...
        try:
            ...
            for symbol in tickers:
                try:
                    ...
                    # جلب بيانات ما قبل السوق (آخر يومين، 1 دقيقة)
                    df_pre = tk.history(period="2d", interval="1m", prepost=True)
        ...
        try:
            ...
            for symbol in tickers:
                try:
                    ...
                    import yfinance as yf
                    tk  = yf.Ticker(symbol)
                    df  = tk.history(period="2d", interval="1m", prepost=True)
                    if df.empty or len(df) < 5:
                        continue
                    ...
                    # حجم ما بعد الإغلاق — 50,000 على الأقل
...
if __name__ == "__main__":
    ...
    threading.Thread(target=rss_catalyst_scanner, daemon=True).start()  # 🔥 ماسح المحفزات الخبري
    threading.Thread(target=form4_insider_scanner, daemon=True).start()  # 🐋 ماسح شراء الداخليين
    threading.Thread(target=pdufa_biotech_scanner, daemon=True).start()  # 💊 ماسح مواعيد FDA/PDUFA
    threading.Thread(target=closing_hours_scanner, daemon=True).start()  # ⏳ ماسح آخر ساعتين
    threading.Thread(target=opening_opportunity_scanner, daemon=True).start()  # 🔔 ماسح فرصة الافتتاح
    threading.Thread(target=premarket_scanner, daemon=True).start()  # 🌅 ماسح ما قبل الافتتاح
    threading.Thread(target=after_hours_scanner, daemon=True).start()  # 🌙 ماسح ما بعد الإغلاق
    threading.Thread(target=penny_stock_scanner, daemon=True).start()   # 💎 ماسح Penny Stocks
