...
def fetch_from_yahoo(symbol, period="5d", interval="15m", timeout=10):
    """تجلب البيانات باستخدام مكتبة yfinance لتقليل احتمالية الحظر (Rate Limit)."""
    try:
        # إضافة تأخير عشوائي أطول بين الطلبات (1.5 - 3.0 ثواني) لحماية الـ IP
        time.sleep(random.uniform(1.5, 3.0))
        
        ticker = yf.Ticker(symbol)
        # auto_adjust=True يضمن أن الأسعار معدلة للانقسامات والتوزيعات
        ...
        # توحيد أسماء الأعمدة للصغيرة لتتوافق مع باقي الكود
        df.columns = [c.lower() for c in df.columns]
        ...
        try:
            ...
            for symbol in tickers:
                try:
                    ...
                    # yf is imported globally at the top
                    tk = yf.Ticker(symbol)
...
if __name__ == "__main__":
    ...
    threading.Thread(target=support_resistance_scanner, daemon=True).start()  # 📍 ماسح الدعم والمقاومة اللحظي
