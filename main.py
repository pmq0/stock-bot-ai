...
def full_sweep_scanner():
    ...
    BATCH_SIZE    = 30     # دفعات أصغر لتقليل ضغط Yahoo
    MIN_GAIN_PCT  = 20.0   # حد أدنى 20% للتنبيه
    SWEEP_SLEEP   = 1.0    # ثانية بين الدفعات لتخفيف ضغط Yahoo
    # FULL_COOLDOWN يتحدد ديناميكياً: 180s نهار، 600s ليل
    ALERT_COOLDOWN = 3600  # ساعة بين تنبيهات نفس السهم
    FULL_COOLDOWN  = 300   # القيمة الافتراضية (تُعاد تحديداً في الحلقة)
    ...
    while True:
        try:
            ...
            if phase == "CLOSED":
                FULL_COOLDOWN = 900  # 15 دقيقة في الليل
            else:
                FULL_COOLDOWN = 300  # 5 دقائق في ساعات التداول
...
def top_gainers_scanner():
    ...
    _sent_today: set = set()  # منع تكرار السهم في اليوم نفسه
    _sent_date = None
    ALERT_COOLDOWN = 3600   # كولداون تقني إضافي، مع منع يومي نهائي
    ...
    while True:
        try:
            ...
            if _sent_date != now_est_date:
                _sent_today.clear()
                _sent_date = now_est_date
            saudi_t  = now_saudi().strftime("%I:%M %p")
            ...
            for q in quotes:
                try:
                    ...
                    # منع تكرار السهم في اليوم نفسه، حتى لو انتهى كولداون الساعة
                    daily_key = f"{symbol}_{now_est_date}"
                    if daily_key in _sent_today:
                        continue
...
def hot_watchlist_scanner():
    """يفحص أفضل المرشحين بعمق كل 15 ثانية ويرسل ثلاثة مستويات تنبيه."""
    sent_levels = {"early": {}, "pre_breakout": {}, "breakout": {}}
    while True:
        try:
            scanner_heartbeat("early_hunter_sweep")
            if not EARLY_HUNTER_ENABLED or get_market_phase() == "CLOSED":
                time.sleep(60)
                continue
            with state_lock:
                candidates = list(state.get("hot_watchlist", []))[:EARLY_HUNTER_MAX_DEEP]
            if not candidates:
                time.sleep(30)
                continue
            now_ts = time.time()
            for candidate in candidates:
                ...
                try:
                    ...
                    rsi = float(df["rsi"].iloc[-1]) if "rsi" in df else 50.0
                    vwap = float(df["vwap"].iloc[-1]) if "vwap" in df else price
                    atr = float(calculate_atr(df) or 0.0)
                    whale = bool(detect_whale_accumulation(df))
                    squeeze = bool(detect_bb_squeeze(df))
                    silent_result = detect_silent_accumulation(df)
                    silent_acc = bool(silent_result[0]) if isinstance(silent_result, tuple) else bool(silent_result)
                    closes = df["close"].tail(5).astype(float).tolist()
                    higher_lows = len(closes) == 5 and all(closes[i] < closes[i + 1] for i in range(4))
                    resistance = price >= float(df["high"].tail(10).max()) * 0.97
                    score = 0
                    details = []
                    if rvol >= 3: score += 25; details.append(f"RVOL {rvol:.1f}x")
                    elif rvol >= 2: score += 15; details.append(f"RVOL {rvol:.1f}x")
                    if 45 <= rsi <= 65: score += 15; details.append(f"RSI {rsi:.0f}")
                    if price > vwap: score += 15; details.append("فوق VWAP")
                    if whale: score += 20; details.append("تجميع سيولة")
                    if squeeze: score += 10; details.append("BB Squeeze")
                    if silent_acc: score += 10; details.append("تجميع صامت")
                    if higher_lows: score += 10; details.append("Higher Lows")
                    if resistance: score += 10; details.append("قرب مقاومة")
                    score = min(100, score)
                    if score >= 90 and rvol >= 3 and price > vwap:
                        level = "🔴 BREAKOUT"; bucket = "breakout"
                    elif score >= 80 and rvol >= 2.5:
                        level = "🟠 PRE-BREAKOUT"; bucket = "pre_breakout"
                    elif score >= 70:
                        level = "🟡 EARLY"; bucket = "early"
                    else:
                        continue
                    if now_ts - sent_levels[bucket].get(symbol, 0) < EARLY_HUNTER_ALERT_COOLDOWN:
                        continue
                    if send_breakout_alert(symbol, price, score, rvol, level, details, atr):
                        sent_levels[bucket][symbol] = now_ts
                except Exception as error:
                    logger.debug(f"[HotWatchlist] {symbol}: {error}")
                    continue
            time.sleep(EARLY_HUNTER_HOT_INTERVAL)
        except Exception as error:
            logger.error(f"[HotWatchlist] error: {error}")
            time.sleep(30)
...
if __name__ == "__main__":
    ...
    threading.Thread(target=early_hunter_sweep_loop, daemon=True).start()
    threading.Thread(target=hot_watchlist_scanner, daemon=True).start()
    threading.Thread(target=pre_explosion_scanner_v3, daemon=True).start()
    threading.Thread(target=vwap_bounce_scanner, daemon=True, name='VWAPBounceScanner').start()
    threading.Thread(target=candle_pattern_scanner, daemon=True, name='CandlePatternScanner').start()
    threading.Thread(target=chart_pattern_scanner, daemon=True, name='ChartPatternScanner').start()
    threading.Thread(target=scanner_watchdog, daemon=True, name="ScannerWatchdog").start()
    ...
    threading.Thread(target=bearish_candle_scanner, daemon=True).start()
    threading.Thread(target=premarket_flash_scanner, daemon=True).start()
    ...
    threading.Thread(target=early_hunter_scanner, daemon=True).start()
    threading.Thread(target=second_wave_scanner, daemon=True).start()
    threading.Thread(target=after_hours_breakout_scanner, daemon=True).start()
