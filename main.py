    ...
    while True:
        try:
            ...
            for c in candidates[:30]:
                ...
                price = float(df['close'].iloc[-1])
                if price > 15.0: continue # Gold Hunter Penny Filter
                ...
                tech_score, reasons, risks = 0, [], []
                if rvol >= 3: tech_score += 25; reasons.append(f"🔥 RVOL {rvol:.1f}x")
                if price > vwap: tech_score += 20; reasons.append("✅ فوق VWAP")
                if ema9 > ema21: tech_score += 15; reasons.append("📈 EMA Bullish")
                if 40 <= rsi <= 70: tech_score += 10; reasons.append(f"📊 RSI {rsi:.0f}")

                # Stage 2: Fundamental check only for technical movers
                short_pct, market_cap = 0.0, 0.0
                if tech_score >= 65:
                    try:
                        time.sleep(5.0) # Avoid Yahoo 429
                        ticker = yf.Ticker(symbol)
                        info = ticker.info
                        short_pct = float(info.get('shortPercentOfFloat', 0) or 0) * 100 if info.get('shortPercentOfFloat') is not None else 0.0
                        market_cap = float(info.get('marketCap', 0) or 0) if info.get('marketCap') is not None else 0.0
                        ...
                        if short_pct > 20: tech_score += 15; reasons.append(f"📉 Short {short_pct:.1f}%")
                        if market_cap < 500_000_000: tech_score += 10; reasons.append("🎯 Small Cap")
                        if cash < 50_000_000 and debt > cash: risks.append("💸 نقد منخفض"); tech_score -= 10
                        if profit_margin < 0: risks.append("📉 غير مربحة"); tech_score -= 5
                    except Exception as e:
                        logger.debug(f"AutoHunter info fetch error for {symbol}: {e}")

                ranked.append({
                    'symbol': symbol, 'price': price, 'score': max(0, min(100, tech_score)),
            ...
            for candidate in ranked[:AUTO_HUNTER_MAX_CANDIDATES]:
                if candidate['score'] < AUTO_HUNTER_MIN_SCORE or candidate['market_cap'] == 0: continue
                symbol = candidate['symbol']
                ...
                cap_val = candidate['market_cap']
                short_val = candidate['short_pct']
                cap_str = f"${cap_val:,.0f}" if cap_val > 0 else "N/A"
                short_str = f"{short_val:.1f}%" if short_val > 0 else "N/A"
                ...
                msg += f"   • Gemini: ✅ فحص السيولة\n"
                msg += f"   • Mistral: ✅ تدقيق المخاطر\n"
                msg += f"   • Groq: ✅ تحليل الخبر\n"
