...
SCAN_INTERVAL_SEC = 900  # 15 دقيقة (تمت استعادته من النسخة المستقرة)
BREAK_BETWEEN_CHUNKS = 5
DEEP_ANALYSIS_WORKERS = 4
SIGNAL_COOLDOWN = 7200
MAX_DAILY_SIGNALS = 70        # الحد الأقصى للرسائل يومياً (لتجنب الإزعاج)
...
def fast_filter(symbol):
    """فلتر قوي مستوحى من النسخة المستقرة v20 لتقليل الطلبات وحماية الـ IP"""
    try:
        # استخدام فترة 15 دقيقة للفلترة السريعة
        df = cached_download(symbol, period="1d", interval="15m")
...
def fetch_from_yahoo(symbol, period="5d", interval="15m", timeout=10):
    ...
    try:
        # ✅ زيادة التأخير العشوائي لتجنب الحظر (2.0 - 4.0 ثواني)
