        ...
        try:
            ...
            for candidate in candidates[:20]:
                try:
                    ...
                    if advanced_indicator_score >= 55:
                        score += 10; factors.append(f'Advanced {advanced_indicator_score}/100')
                    ranked.append({
                        'symbol': symbol, 'price': price, 'change': day_change,
                        'rvol': rvol, 'rsi': rsi, 'vwap': vwap,
                        'above_vwap': above_vwap, 'ema_bullish': ema_bullish,
                        'volume_spike': volume_spike, 'breakout': breakout,
                        'score': min(100, score), 'advanced_score': advanced_indicator_score, 'advanced_indicators': advanced_indicators, 'factors': factors, 'timestamp': time.time(),
                    })
                except Exception as error:
