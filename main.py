...
def full_sweep_scanner():
    ...
    FULL_COOLDOWN = 180   # 3 دقائق بين كل دورة كاملة
    ...
    while True:
        try:
            ...
            logger.info(f"[FullSweep] بدء مسح {len(all_tickers)} سهم دفعات {BATCH_SIZE}...")

            alerts_sent = 0

            # في Pre-Market نستخدم بيانات 5m مع prepost=True لصيد حركات 4AM+
            use_prepost = (phase == "PRE")
            dl_interval = "5m" if use_prepost else "1d"
            dl_period   = "2d" if use_prepost else "2d"
