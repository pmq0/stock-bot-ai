...
# ================= DATA FETCHER =================
def cached_download(symbol, period="5d", interval="15m", timeout=10):
...
def update_all_tickers():
    ...
    # إذا فشل جلب البيانات، استخدم القائمة الاحتياطية
    logger.warning(f"Failed to fetch universe. Using minimal universe ({len(MINIMAL_UNIVERSE)} symbols)")
    with state_lock:
...
if __name__ == "__main__":
    ...
    threading.Thread(target=momentum_breaker_scanner, daemon=True).start()  # 🎯
    threading.Thread(target=snap_scanner,             daemon=True).start()  # ⚡
    threading.Thread(target=pre_blast_scanner,        daemon=True).start()  # 💣
...
def cached_download(symbol, period="5d", interval="15m", timeout=10, force_refresh=False):
    """
    نفس safe_download ولكن مع طبقة تخزين مؤقت (Cache) لتقليل الطلبات المكررة.
    - TTL يختلف حسب الفاصل الزمني (1m, 5m, 15m, 1d)
    - force_refresh = True يجبر جلب بيانات جديدة
    """
    cache_key = f"{symbol}_{period}_{interval}"
    
    # إذا لم نطلب تحديثاً إجبارياً، نتحقق من الـ Cache
    if not force_refresh:
        with _cache_lock:
            if cache_key in _cache:
                cached_data, timestamp = _cache[cache_key]
                # حساب مدة صلاحية Cache بناءً على الفاصل الزمني
                ttl = CACHE_TTL.get(interval, 60)  # الافتراضي 60 ثانية
                if time.time() - timestamp < ttl:
                    logger.debug(f"Cache HIT: {symbol} ({interval})")
                    # نعيد نسخة من البيانات لتجنب التعديل غير المقصود
                    return cached_data.copy() if not cached_data.empty else pd.DataFrame()
    
    # إذا لم يكن في Cache أو انتهت صلاحيته، نجلبه من Yahoo
    logger.debug(f"Cache MISS: {symbol} ({interval}) - fetching fresh data")
    df = cached_download(symbol, period, interval, timeout)
    
    # نخزن البيانات الجديدة في Cache
    with _cache_lock:
...
def clear_old_cache():
    """تنظيف الـ Cache من البيانات القديمة - يُستدعى كل 5 دقائق"""
    now = time.time()
    with _cache_lock:
        old_keys = []
        for key, (_, timestamp) in _cache.items():
            # استخراج الفاصل الزمني من المفتاح
            parts = key.split('_')
            interval = parts[2] if len(parts) > 2 else "15m"
            ttl = CACHE_TTL.get(interval, 60)
            if now - timestamp > ttl * 2:  # نحذف بعد ضعف المدة
                old_keys.append(key)
