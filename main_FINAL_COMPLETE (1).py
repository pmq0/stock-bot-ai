...
def safe_download(symbol, period="1d", interval="5m", timeout=10):
    """Download data safely with caching"""
    try:
...
def get_full_universe():
    """جلب 7000+ سهم من NASDAQ + NYSE + AMEX"""
    tickers = set()
    ...
    try:
        ...
        if resp.status_code == 200:
            for line in resp.text.split('\n')[1:]:
                ...
                if parts and len(parts[0]) > 0 and parts[0].isalpha():
                    tickers.add(parts[0].upper())
    except Exception as e:
        logger.debug(f"Error fetching NASDAQ: {e}")
    ...
    result = list(tickers)[:MAX_TICKERS_TO_SCAN]
    logger.info(f"✅ Loaded {len(result)} tickers from universe")
    return result


def fast_filter(symbol):
    """فلتر سريع جداً - أقل من 0.3 ثانية لكل سهم"""
    try:
        df = cached_download(symbol, period="1d", interval="5m", timeout=5)
        ...
        # فقط أسهم تحت 30 دولار - هذا ما تريد لصيد البيني
        ...
        logger.debug(f"fast_filter error for {symbol}: {e}")
        return False
...
def update_all_tickers():
    ...
    # استخدم get_full_universe بدلاً من get_dynamic_tickers
    tickers = get_full_universe()
    
    if not tickers:
        logger.warning("⚠️ Could not fetch tickers")
        return
...
# ================= EXPLOSIVE RVOL SCANNER =================
EXPLOSIVE_MIN_PRICE = 0.3
EXPLOSIVE_MAX_PRICE = 20.0
EXPLOSIVE_MIN_RVOL = 8.0
EXPLOSIVE_TARGET_RVOL = 15.0
EXPLOSIVE_MAX_TICKERS = 300

def explosive_rvol_scanner():
    """ماسح سريع جداً - يصيد الأسهم المنفجرة (RVOL 10x-20x)"""
    while True:
        try:
            phase = get_market_phase()
            ...
            with state_lock:
                tickers = list(state.get("tickers", []))[:EXPLOSIVE_MAX_TICKERS]
            ...
            for symbol in tickers:
                try:
                    ...
                    if price < EXPLOSIVE_MIN_PRICE or price > EXPLOSIVE_MAX_PRICE:
                        continue
                    
                    today_vol = df['volume'].iloc[-6:].sum()
                    yesterday_vol = df[df.index.date < now_est().date()]['volume'].sum()
                    
                    if yesterday_vol <= 0:
                        continue
                    
                    rvol = today_vol / yesterday_vol * 12
                    
                    if rvol < EXPLOSIVE_MIN_RVOL:
                        continue
                    
                    signal_key = f"explosive_{symbol}_{now_est().date()}"
                    with state_lock:
                        if signal_key in state.get("seen_signals", {}):
                            continue
                    
                    entry = price
                    tp1 = round(entry * 1.15, 4)
                    tp2 = round(entry * 1.35, 4)
                    tp3 = round(entry * 1.60, 4)
                    sl = round(entry * 0.92, 4)
                    
                    if rvol >= 20:
                        strength = "🚨 انفجار هائل (20x+)"
                    elif rvol >= 15:
                        strength = "🔥 انفجار عنيف (15x+)"
                    elif rvol >= 10:
                        strength = "⚡ انفجار قوي (10x+)"
                    else:
                        strength = "📈 نشاط استثنائي"
                    
                    msg = (
                        f"💥 *EXPLOSIVE RVOL: {symbol}*\n"
                        f"━━━━━━━━━━━━━━━━\n"
                        f"💰 السعر: *${entry:.4f}*\n"
                        f"🔥 RVOL: *{rvol:.1f}x* {strength}\n"
                        f"━━━━━━━━━━━━━━━━\n"
                        f"🎯 *الأهداف:*\n"
                        f"  • T1: ${tp1} (+15%)\n"
                        f"  • T2: ${tp2} (+35%)\n"
                        ...
                        f"🛑 وقف الخسارة: ${sl} (-8%)\n"
                        f"━━━━━━━━━━━━━━━━\n"
                        f"💡 *توقع حركة عنيفة خلال الساعة القادمة*\n"
                        f"⚠️ *هذا سهم شديد الخطورة - إدارة المخاطر إجبارية*"
                    )
                    ...
                    logger.info(f"💥 EXPLOSIVE: {symbol} RVOL={rvol:.1f}x")
                    time.sleep(2)
                    
                except Exception as e:
                    logger.debug(f"Explosive scan error {symbol}: {e}")
                    continue
                time.sleep(0.2)
                
        except Exception as e:
            logger.error(f"Explosive scanner error: {e}")
        time.sleep(90)


def get_dynamic_tickers():
    """جلب الأسهم النشطة ديناميكياً - Top Gainers + Most Active"""
    tickers = set()
    
    try:
        # 1. Top Gainers من Yahoo Finance
    ...
    try:
        ...
        logger.debug(f"Error fetching most active: {e}")
    ...
    if len(tickers) < 20:
        try:
            ...
            logger.debug(f"Error fetching from API: {e}")
    ...
    result = list(tickers)[:500]
    logger.info(f"✅ Dynamic universe: {len(result)} active stocks")
    return result


def quick_pre_pump_filter(symbol):
    """
    فلتر خفيف جداً - أقل من 0.3 ثانية لكل سهم
    يمرر فقط الأسهم اللي فيها بوادر انفجار وشيك
    """
    try:
        ticker = yf.Ticker(symbol)
        ...
        # فلتر 1: سعر منخفض (بيني ستوكس)
        if price < 0.3 or price > 20:
            return False
        
        # فلتر 2: حجم متزايد (أهم مؤشر للانفجار)
        vol_last_3 = df['volume'].iloc[-3:].mean()
        vol_avg_20 = df['volume'].rolling(20).mean().iloc[-1]
        
        if vol_last_3 < vol_avg_20 * 1.5:
            return False
        
        # فلتر 3: السعر يبدأ بالتحرك
...
def scan_all_stocks_for_pre_pump():
    """مسح كل الأسهم النشطة في 30-40 دقيقة"""
    
    with state_lock:
        all_tickers = state.get("tickers", [])
    
    if not all_tickers:
        all_tickers = update_all_tickers()
        if not all_tickers:
            return []
    
    logger.info(f"🔍 Scanning {len(all_tickers)} stocks for pre-pump signals...")
    
    candidates = []
    
    # دفعات متوازية - 10 عمال
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(quick_pre_pump_filter, sym): sym for sym in all_tickers}
    ...
    logger.info(f"🎯 Pre-pump candidates: {len(candidates)} stocks")
    
    # تحليل عميق للمرشحين فقط
    results = []
...
if __name__ == "__main__":
    ...
    threading.Thread(target=explosive_rvol_scanner, daemon=True).start()
