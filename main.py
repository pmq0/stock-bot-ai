    ...
    for source_url in ticker_sources:
        try:
            ...
            if response.status_code == 200:
                ...
                temp_tickers = []
                for line in lines:
                    # تنظيف السطر واستخراج الرمز
                    line = line.strip().upper()
                    if not line or line.startswith('SYMBOL') or line.startswith('TICKER'):
                        continue
                ...
                if len(temp_tickers) > 100:
                    tickers = temp_tickers
                    logger.info(f"✅ Got {len(tickers)} tickers from {source_url}")
                    break  # نوقف المحاولات بعد أول مصدر ناجح
                else:
                    logger.warning(f"Only {len(temp_tickers)} tickers from {source_url}, trying next...")
