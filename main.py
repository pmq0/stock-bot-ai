...
# ✨ تحسينات إضافية (Anti-pump, Multi-TF, ORB, VWAP Reclaim, Finnhub)
from enhancements import (
    finnhub,
    is_overextended,
    is_multi_timeframe_aligned,
    sector_strength_gate,
    is_near_earnings,
    quality_gate,
    opening_range_breakout_scanner,
    vwap_reclaim_scanner,
)
...
PHASE_SETTINGS = {
    ...
    "REGULAR": {"min_score": 65,  "size_multiplier": 1.0, "vol_surge_mult": 1.2, "description": "🟢 Regular Hours"},
    "AFTER":   {"min_score": 75,  "size_multiplier": 0.3, "vol_surge_mult": 2.0, "description": "🔵 After-Hours"},
...
def process_symbol(symbol):
    try:
        ...
        if score >= effective_min_score:
            # ✨ بوابة الجودة الموحدة (Anti-pump + Multi-TF + Sector + Earnings)
            try:
                df_daily_for_gate = cached_download(symbol, period="60d", interval="1d")
                if not df_daily_for_gate.empty:
                    df_daily_for_gate.columns = [c.lower() for c in df_daily_for_gate.columns]
            except Exception:
                df_daily_for_gate = None

            # في PRE/AFTER نكون متساهلين، في REGULAR نكون متشددين
            strict_mode = (phase == "REGULAR")
            passed, _, fail_reasons = quality_gate(
                symbol, df, cached_download,
                df_daily=df_daily_for_gate,
                strict=strict_mode,
            )
            if not passed:
                logger.info(f"Skip {symbol}: quality gate failed — {fail_reasons}")
                return
...
if __name__ == "__main__":
    ...
    # ✨ ماسحات إضافية (مجانية، خفيفة، للأسهم القوية)
    threading.Thread(
        target=opening_range_breakout_scanner,
        args=(state, state_lock, cached_download, send_telegram,
              get_market_phase, now_est, save_state),
        daemon=True,
    ).start()
    threading.Thread(
        target=vwap_reclaim_scanner,
        args=(state, state_lock, cached_download, send_telegram,
              get_market_phase, now_est, save_state),
        daemon=True,
    ).start()
    ...
    if finnhub.enabled:
        logger.info("✨ Finnhub enabled — Real Float, Earnings, Sector data active")
    else:
        logger.info("ℹ️ Finnhub disabled — set FINNHUB_TOKEN env to enable real data")
