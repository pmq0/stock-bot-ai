...
def update_all_tickers():
    ...
    tickers = []
    try:
        logger.info("🔄 Fetching from NASDAQ HTTP (Better for Railway)...")
        # استخدام HTTP بدلاً من FTP لتجنب مشاكل الحجب في Railway
        ...
        # تحميل بيانات ناسداك
        response_nasdaq = requests.get(nasdaq_url)
        if response_nasdaq.status_code == 200:
            df_nasdaq = pd.read_csv(io.StringIO(response_nasdaq.text), sep="|")
            tickers.extend(df_nasdaq["Symbol"].dropna().tolist())
            
        # تحميل بيانات البورصات الأخرى
        response_other = requests.get(other_url)
        if response_other.status_code == 200:
            df_other = pd.read_csv(io.StringIO(response_other.text), sep="|")
...
# ================= TRADE MANAGEMENT =================
def open_trade(symbol, price, score, df, is_accumulating=False, is_pre_breakout=False, phase="REGULAR", settings=None, rvol=1.0, low_float=False, is_bullish=True, is_pdh=True, is_silent_acc=False, is_raw_mom=False, psych_level=0, gap_pct=0, is_v_shape=False):
    with state_lock:
