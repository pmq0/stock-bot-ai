...
# لو ربطت Volume دائم على Railway، RAILWAY_VOLUME_MOUNT_PATH يتحدد تلقائيًا —
# الملف يترحّل له فيعيش بعد أي إعادة تشغيل. بدون Volume، يشتغل زي ما كان (محلي، ينمسح مع الحاوية).
STATE_FILE = os.path.join(os.getenv("RAILWAY_VOLUME_MOUNT_PATH", "."), "state_penny_hunter.json")
...
Ranking+AI، AUTO HUNTER المُثرى (سكور 80+)، Mega Mover (حركة كبيرة حتى لو هدت)، Gap Hunter (فجوة ≥20% عن إغلاق أمس)، دوجي بعد هبوط (بتأكيد اليوم اللي بعده)، وأخبار Finnhub مرفقة تلقائيًا بأي تنبيه كبير
...
def get_news_context_line(symbol):
    ...
    if best.get("sentiment", 0) < 0.65:
        return None
    headline = str(best.get("headline", ""))[:90]
    return f"📰 خبر مؤكد: {headline} ({best.get('sentiment', 0) * 100:.0f}% إيجابي)"


def _finnhub_current_price(symbol):
    """
    كانت تقرأ Yahoo فقط رغم اسمها (نفس تأخير كل مكان ثاني). الحين تجرب Finnhub
    الحقيقي أول (quote() — مجاني أصلًا، بدون حساب أو تكلفة جديدة، وأسرع من
    Yahoo المؤجّل)، وترجع لـYahoo فقط لو Finnhub غير مفعّل أو فشل الطلب.
    """
...
def _send_hunter_style_alert(symbol, res, candidate, info_calls_box):
    ...
    msg += (f"\n🎯 *خطة التداول:*\n   🎯 هدف 1: ${fresh_price * 1.08:.4f} (+8%)\n"
            f"   🎯 هدف 2: ${fresh_price * 1.15:.4f} (+15%)\n   🛑 وقف: ${fresh_price * 0.95:.4f} (-5%)\n")
...
def doji_reversal_scanner():
    """
    دوجي بعد هبوط: شمعة يومية جسمها صغير جدًا (مفرغة/متردّدة) بعد
    DOJI_MIN_DOWN_DAYS يوم هبوط متتالي = "مرشح" فقط. ما يتحول لتنبيه فعلي
    إلا لو اليوم اللي بعده أغلق فوق قمة شمعة الدوجي (تأكيد صعودي حقيقي) —
    نفس فلسفة arm/confirm بالـESE، عشان ما نشتري دوجي لحاله بدون تأكيد.
    """
    if not DOJI_ENABLED:
    ...
    while True:
        try:
            ...
            for symbol in symbols:
                try:
                    df = cached_download(symbol, period="1mo", interval="1d")
                    ...
                    if rec and rec.get("armed_date") and rec["armed_date"] != today_key and not rec.get("sent"):
                        ...
                        if confirm_close > _safe_float(rec.get("doji_high")):
                            msg = (f"✨ *تأكيد نمط دوجي: {symbol}*\n━━━━━━━━━━━━━━━━\n"
                                   f"دوجي بتاريخ {rec['armed_date']} بعد {DOJI_MIN_DOWN_DAYS} أيام هبوط متتالية،\n"
                                   f"واليوم أغلق فوق قمة الدوجي (${_safe_float(rec.get('doji_high')):.4f}) عند *${confirm_close:.4f}*.\n")
                            news_line = get_news_context_line(symbol)
                            if news_line:
                                msg += f"\n{news_line}\n"
                    ...
                    if is_doji and down_run and (not rec or rec.get("armed_date") != today_key):
                        ...
                        with state_lock:
                            state.setdefault("doji_watch", {})[symbol] = {
                                "armed_date": today_key, "doji_high": doji_high, "sent": False,
                            }
            ...
            with state_lock:
                ...
                save_state()
            logger.info(f"[Doji] scanned={len(symbols)} watching={len(dw)}")
    ...
    while True:
        try:
            ...
            for c in candidates:
                ...
                if prev and change < prev_change + GAP_HUNTER_RESEND_DELTA:
                    continue
                price = _finnhub_current_price(symbol) or _safe_float(c.get("price"))
                ...
                send_telegram(msg)
                entry = {"change": change, "ts": time.time()}
                with state_lock:
                ...
                if sent_count >= GAP_HUNTER_MAX_PER_CYCLE:
                    break
                time.sleep(1)
            logger.info(f"[GapHunter] pool={len(candidates)} sent={sent_count}")
...
GAP_HUNTER_RESEND_DELTA  = float(os.getenv("GAP_HUNTER_RESEND_DELTA", "15"))    # إعادة إرسال لو الفجوة كبرت بهالقد عن آخر تنبيه لنفس السهم اليوم
...
if __name__ == "__main__":
    ...
    # 🚀 MEGA MOVER + ✨ DOJI REVERSAL + 🕳️ GAP HUNTER — طبقات توعية إضافية، منفصلة عن صرامة الدخول
    threading.Thread(target=mega_mover_scanner, daemon=True, name="MegaMover").start()
    threading.Thread(target=doji_reversal_scanner, daemon=True, name="DojiReversal").start()
    threading.Thread(target=gap_hunter_scanner, daemon=True, name="GapHunter").start()
    logger.info('🚀 Mega Mover + ✨ Doji Reversal + 🕳️ Gap Hunter started')
