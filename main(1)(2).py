                ...
                try:
                    ...
                    for entry in feed.entries[:15]:
                        try:
                            ...
                            with state_lock:
                                if signal_key in state.get(\"seen_signals\", {}):
                                    logger.debug(f\"⏭️ تم تجاهل الخبر المكرر: {symbol}\")
                                    continue
                            ...
                            try:
                                ...
                                if df.empty or len(df) < 10:
                                    logger.debug(f\"⚠️ بيانات ناقصة للسهم: {symbol}\")
                                    continue
                                ...
                                if current_price < STRONG_NEWS_PRICE_MIN:
                                    logger.debug(f\"⚠️ سعر السهم {symbol} منخفض جداً: ${current_price:.2f}\")
                                    continue
                                ...
                                if rvol < STRONG_NEWS_RVOL_MIN:
                                    logger.debug(f\"⚠️ الحجم النسبي منخفض للسهم {symbol}: {rvol:.2f}x\")
                                    continue
                    ...
                    logger.warning(f\"⚠️ خطأ في قراءة feed {feed_url}: {feed_err}\")
                    continue
