...
def call_openrouter(prompt, max_tokens=300):
    ...
    try:
        response = requests.post(
            ...
            json={'model': 'google/gemini-2.0-flash-exp:free', 'messages': [{'role': 'user', 'content': prompt}], 'max_tokens': max_tokens},
            timeout=10,
        )
        if response.status_code != 200:
...
def full_sweep_scanner():
    ...
    while True:
        try:
            ...
            if not all_tickers:
                all_tickers = ['AAPL', 'MSFT', 'NVDA', 'AMD', 'TSLA', 'AMZN', 'META', 'PLTR', 'SOFI', 'MARA', 'RIOT', 'IONQ']
                logger.warning('[Discovery] No tickers available; using fallback universe=%d', len(all_tickers))
...
def hot_watchlist_scanner():
    ...
    while True:
        try:
            ...
            for item in candidates:
                symbol = str(item.get('symbol', '')).upper().strip()
                if symbol and symbol not in KNOWN_DELISTED:
                    unique[symbol] = item
            candidates = sorted(unique.values(), key=lambda item: (
                float(item.get('change', 0) or 0), float(item.get('rvol', 0) or 0),
            ), reverse=True)[:20]
            ...
            for candidate in candidates:
                ...
                try:
                    ...
                    # Discovery fallback: keeps strong candidates rankable when Yahoo is temporarily unavailable.
                    if df is None or df.empty or len(df) < 20:
                        fallback_score = 0
                        ...
                        if fallback_score < 35:
                            rejected += 1
                            logger.info('[Ranking] %s rejected: insufficient Yahoo data fallback_score=%d', symbol, fallback_score)
                            continue
                        ranked.append({'symbol': symbol, 'price': candidate_price, 'change': candidate_change,
                                       'rvol': candidate_rvol, 'rsi': 50.0, 'vwap': candidate_price,
                                       'above_vwap': True, 'ema_bullish': True, 'volume_spike': candidate_rvol >= 2,
                                       'breakout': True, 'score': min(75, max(45, fallback_score)),
                                       'advanced_score': 0, 'advanced_indicators': None,
                                       'factors': ['Discovery fallback', f'+{candidate_change:.1f}%'],
                                       'timestamp': time.time(), 'data_source': 'DISCOVERY_FALLBACK'})
                        logger.warning('[Ranking] %s Yahoo unavailable → fallback ranking', symbol)
                        continue
                    ...
                    if not required.issubset(df.columns):
                        rejected += 1
                        logger.info('[Ranking] %s rejected: missing columns', symbol)
                        continue
                    df = compute_indicators(df)
                    if df.empty or len(df) < 20:
                        rejected += 1
                    ...
                    advanced_indicator_score = 0
                    try:
                        if len(df) >= 60:
                            advanced_indicators = analyze_advanced_indicators(symbol, df)
                            if isinstance(advanced_indicators, dict):
                                advanced_indicator_score = int(advanced_indicators.get('score', 0) or 0)
                    except Exception as advanced_error:
                        logger.debug('[Ranking] %s advanced indicators skipped: %s', symbol, advanced_error)

                    price = float(df['close'].iloc[-1])
                    if not (0.30 <= price <= MAX_PRICE):
                        rejected += 1
                        continue
                    ...
                    if day_change >= 20: score += 20; factors.append(f'+{day_change:.1f}%')
                    elif day_change >= 10: score += 15; factors.append(f'+{day_change:.1f}%')
                    elif day_change >= 5: score += 8
                    elif day_change >= 3: score += 5
                    ...
                    ranked.append({'symbol': symbol, 'price': price, 'change': day_change, 'rvol': rvol, 'rsi': rsi,
                                   'vwap': vwap, 'above_vwap': above_vwap, 'ema_bullish': ema_bullish,
                                   'volume_spike': volume_spike, 'breakout': breakout, 'score': min(100, int(score)),
                                   'advanced_score': advanced_indicator_score, 'advanced_indicators': advanced_indicators,
                                   'factors': factors, 'timestamp': time.time(), 'data_source': 'YAHOO_5M'})
                except Exception as error:
                    rejected += 1
                    logger.warning('[Ranking] %s skipped: %s', symbol, error)
            ...
            for candidate in top5:
                if ai_count >= 3 or int(candidate.get('score', 0)) < 65:
                    break
                symbol = candidate['symbol']
                if time.time() - last_ai_time.get(symbol, 0) < 3600:
                    continue
                last_ai_time[symbol] = time.time()
                if AI_PROCESS_ENABLED:
                    threading.Thread(target=final_trade_recommendation,
                                     args=(symbol, 0, 'TOP_GAINER_RANKED', False, True),
                                     daemon=True, name=f'TopRankAI-{symbol}').start()
                    ai_count += 1
                    time.sleep(1)
            logger.info('[Ranking] candidates=%d | ranked=%d | rejected=%d | top=%s | AI=%d',
                        len(candidates), len(ranked), rejected,
                        ', '.join(f"{x['symbol']}:{x['score']}" for x in top5), ai_count)
            time.sleep(30)
        except Exception as error:
            logger.error('[RankingEngine] error: %s', error)
            time.sleep(60)
...
def calculate_ichimoku(df, tenkan_period=9, kijun_period=26, senkou_period=52):
    """يحسب Ichimoku ويعيد وضع السعر بالنسبة للسحابة."""
    try:
        work = df.copy()
        work.columns = [str(c).lower() for c in work.columns]
        if not {'high', 'low', 'close'}.issubset(work.columns) or len(work) < senkou_period:
            return None
        ...
        logger.debug(f'[Ichimoku] {error}')
        return None


def calculate_supertrend(df, period=10, multiplier=3.0):
    """يحسب SuperTrend باتجاه ووقف ديناميكيين."""
    try:
        work = df.copy()
        work.columns = [str(c).lower() for c in work.columns]
        if not {'high', 'low', 'close'}.issubset(work.columns) or len(work) < period + 2:
            return None
        ...
        for i in range(period, len(work)):
            prev = i - 1
            if close.iloc[i] > upper.iloc[prev]:
                direction.iloc[i] = 1
            elif close.iloc[i] < lower.iloc[prev]:
                direction.iloc[i] = -1
            else:
                direction.iloc[i] = direction.iloc[prev]
                if direction.iloc[i] == 1:
                    lower.iloc[i] = max(lower.iloc[i], lower.iloc[prev])
                else:
                    upper.iloc[i] = min(upper.iloc[i], upper.iloc[prev])
        ...
        return {'supertrend': last_line, 'direction': 'UPTREND' if last_dir == 1 else 'DOWNTREND',
                'color': 'GREEN' if last_dir == 1 else 'RED', 'signal': 'BUY' if last_dir == 1 and prev_dir == -1 else 'SELL' if last_dir == -1 and prev_dir == 1 else 'HOLD',
                'price': float(close.iloc[-1])}
    except Exception as error:
        logger.debug(f'[SuperTrend] {error}')
        return None


def calculate_fibonacci(df, lookback=50):
    """يحسب Fibonacci Retracement ومستوى الارتداد الأقرب."""
    try:
        work = df.copy()
        work.columns = [str(c).lower() for c in work.columns]
        if not {'high', 'low', 'close'}.issubset(work.columns) or len(work) < min(lookback, 20):
            return None
        ...
        return {'high': high, 'low': low, 'current': current, 'levels': levels, 'closest_level': closest[0], 'closest_price': closest[1], 'near_618': near_618,
                'signal': 'SUPPORT' if near_618 and current >= levels['61.8%'] else 'RESISTANCE' if near_618 else 'NEUTRAL'}
    except Exception as error:
        logger.debug(f'[Fibonacci] {error}')
        return None


def calculate_keltner_channels(df, period=20, multiplier=2.0):
    """يحسب Keltner Channels مع كشف squeeze."""
    try:
        work = df.copy()
        work.columns = [str(c).lower() for c in work.columns]
        if not {'high', 'low', 'close'}.issubset(work.columns) or len(work) < period + 2:
        ...
        return {'upper': float(upper.iloc[-1]), 'middle': float(middle.iloc[-1]), 'lower': float(lower.iloc[-1]), 'width': width,
                'squeeze': bool(avg_width > 0 and width < avg_width * .7), 'position': position,
                'signal': 'OVERBOUGHT' if position == 'ABOVE_UPPER' else 'OVERSOLD' if position == 'BELOW_LOWER' else 'NEUTRAL'}
    except Exception as error:
        logger.debug(f'[Keltner] {error}')
        return None


def calculate_parabolic_sar(df, step=.02, max_step=.2):
    """يحسب Parabolic SAR بطريقة خفيفة دون اعتماد إضافي."""
    try:
        work = df.copy()
        work.columns = [str(c).lower() for c in work.columns]
        if not {'high', 'low', 'close'}.issubset(work.columns) or len(work) < 5:
            return None
        ...
        for i in range(1, len(close)):
            ...
            if up:
                ...
                if low[i] < candidate:
                    up, sar, extreme, af = False, extreme, low[i], step
                else:
                    sar = candidate
                    if high[i] > extreme:
                        extreme, af = high[i], min(max_step, af + step)
            else:
                candidate = max(candidate, high[i - 1], high[i - 2] if i > 1 else high[i - 1])
                if high[i] > candidate:
                    up, sar, extreme, af = True, extreme, high[i], step
                else:
                    sar = candidate
                    if low[i] < extreme:
                        extreme, af = low[i], min(max_step, af + step)
            series.append(sar)
        price = float(close[-1])
        return {'sar': float(series[-1]), 'direction': 'UPTREND' if price > series[-1] else 'DOWNTREND', 'reversal': False,
                'signal': 'BUY' if price > series[-1] else 'SELL', 'price': price}
    except Exception as error:
        logger.debug(f'[ParabolicSAR] {error}')
        return None


def analyze_advanced_indicators(symbol, df):
    """يجمع المؤشرات الخمسة ويعيد score إضافياً للتحليل والـRanking."""
...
def advanced_analysis(symbol):
    ...
    try:
        ...
        return {
            ...
            'fib': fib, 'above_ema20': price >= ema20, 'advanced_indicators': advanced_indicators,
        }
    except Exception as error:
